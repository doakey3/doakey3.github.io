...
var custom_setSelectionRange = function(textarea, selectionStart, selectionEnd) {
    var top = get_pos(textarea.parentElement).y;
    
    var full_text = textarea.value;
    textarea.value = full_text.substring(0, selectionEnd);
    ...
    var short_height = textarea.parentElement.offsetHeight;
    
    textarea.value = full_text;
    textarea.dispatchEvent(event);
    
    if (top + short_height > (document.body.scrollTop + window.innerHeight)) {
        document.body.scrollTop = (top + short_height) - (window.innerHeight / 2);
    }
    
    else if (top + short_height < document.body.scrollTop) {
        document.body.scrollTop = top + short_height - (window.innerHeight / 2);
    }
...
for (var i = 0; i < checkable_textareas.length; i++) {
    ...
    checkable_textareas[i].addEventListener("keydown", function(event) {
        ...
        if (event.keyCode == tab_key && !event.shiftKey) {
            ...
            if (match) {
                custom_setSelectionRange(this, pre.length + match.index, pre.length + match.index + match[0].length);
            }
            if (!match) {
                var match = /\[[^\[\]]+\]/.exec(this.value);
                if (match) {
                    custom_setSelectionRange(this, match.index, match.index + match[0].length);
                }
            }
        }
            ...
            if (array.length > 0) {
                custom_setSelectionRange(this, array[array.length - 1].index, array[array.length - 1].index + array[array.length - 1][0].length);
            }
